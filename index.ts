...
type NetlifyBody = {
  ...
  config: {
    name: string;
    site_url: string;
    /** may be renamed in future https://netlify.slack.com/archives/DC57GR27P/p1548874982034200 */
    config: {
      repoURL?: string;
    };
  };
...
function useBody(body: NetlifyBody) {
  ...
  */
  ...
  return { userConfig, scriptstring };
}

// Create an instance of your Service for Netlify Site
app.post('/instances', function(req, res) {
  const body: NetlifyBody = req.body;

  const { userConfig, scriptstring } = useBody(body);
...
app.get('/instances/:id', function(req, res) {
  ...
  const { scriptstring } = useBody(body);
